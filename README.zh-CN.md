...
我每天都同时用 Claude Code 和 Codex,但一直被两件事卡住:每当让它们改同一个项目,
我就得在两边来回复制粘贴,还得盯着谁能动哪些文件;而当其中一个用满了套餐额度,
我又希望能把任务直接交接给另一个。CoAct 就是为此而生的协同层——它现在解决了复制
粘贴和互相踩踏的问题,并正朝着"跨套餐交接"演进。
...
写入意图锁、能力策略(受保护路径 + 每个 agent 的写入范围)、任务板、presence 和
journal——全部装在一个跨平台的二进制里。路线图上:带合并闸门的 git worktree 隔离、
更多 agent 适配器、以及可选的消息平面。见
