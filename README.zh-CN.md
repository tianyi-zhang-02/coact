...
```sh
coact             # 打开只监听 127.0.0.1 的本地控制台
```

UI 是普通用户的默认路径:初始化仓库、写项目 brief、创建/领取/完成任务、给 agent 发消息、
查看在线 agent/锁/journal,并复制准确的启动命令。它只做本地轮询,不提供任意 shell
执行接口。

如果你更喜欢终端工作流,原来的 CLI 仍然保留:

```sh
coact init        # 接入 Claude Code 的 hook,并写入 agent 契约
coact doctor      # 验证:检查接线,并对强制执行做自检
coact help        # 查看所有 CLI 命令
```
...
两个 agent 直接对话,不用人来回传:

```sh
coact msg codex "有空帮我看下 auth 的 diff"          # 发送
coact inbox                                          # 读你的消息
coact handoff codex "auth 完成八成,补一下 token 刷新"  # 转交任务 + 通知
```

消息走共享文件系统(受治理、记入 journal)。它是**回合制**的——agent 在每回合开始时
读收件箱——而不是 AgentBridge 那种实时的回合中推送。`coact handoff` 会把你正在做的
任务转给对方、释放你的锁、并带上上下文——这就是"我要停了/额度用完了,你接手"。
...
| `coact msg <to> <text>` / `coact inbox` | 给另一个 agent 发/收消息(受治理、记入 journal) |
| `coact handoff <to>` | 把你的任务和上下文交接给另一个 agent |
| `coact lock <path>` / `unlock <path>` | 写入意图锁(`unlock --all` 释放你所有的锁) |
| `coact policy check <path>` / `show` | 测试或查看写入策略 |
| `coact sidecar` | 单会话的在线心跳 |
| `coact versions` / `update` / `switch <version>` | 管理 `~/.coact` 下的本地版本 |
...
journal、本地控制台、可选的带合并闸门的 git worktree 隔离、回合制的 agent 间消息与
任务交接,以及 `~/.coact` 下的托管版本——全部装在一个跨平台的二进制里。路线图上:
更深的实时(回合中)消息、按额度自动触发的交接,以及更多 agent 适配器。见
...
托管更新会把二进制并排安装到 `~/.coact/bin`,只切换 `~/.coact/coact` 这个 shim:
...
写入被限定在仓库内、agent 标识会被净化、hook **失败开放**,而且一切都能用
`coact deinit` 移除。本地控制台绑定 `127.0.0.1`,并额外强制 Host 头白名单和一次性
CSRF token,因此其他网页无法驱动它。实验性的 `coact update` 是唯一需要联网的可选
功能:它通过 HTTPS 拉取发行版、校验 SHA-256,并且只管理 `~/.coact` 下的文件。完整
的安全模型见 [SECURITY.md](SECURITY.md)。
