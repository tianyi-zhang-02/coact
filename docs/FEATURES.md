...
| Claude/Codex/Gemini native launchers | Ready | Yes | The underlying agent CLI must already be installed and authenticated. |
| Shared team policy and project memory | Ready | Yes | Human-controlled local context; never store secrets. |
| Planning runs and final distributor | Ready | Yes | Agents still need a turn to read inbox and write proposals; CoAct does not autonomously wake them. |
| Board add/claim/done and handoff | Ready | Yes | Ownership is coordinated locally, not synced to an external issue tracker. |
...
| Locks and policy | Ready, asymmetric | Yes | Claude is L2 hook-enforced; Codex/Gemini are L1 contract-enforced in shared mode. |
...
| Local web UI | Experimental/optional | Not required | Terminal-native CLI is the supported primary workflow. |
